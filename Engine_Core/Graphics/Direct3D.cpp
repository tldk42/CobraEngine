...
namespace Cobra
{
	Direct3D::Direct3D(bool InbVsync): bVsync(InbVsync),
	                                   mVideoCardMemory(0),
	...
	bool Direct3D::Initialize(HWND  hWnd, const float  winWidth, const float winHeight, bool fullScreen,
...
#pragma region 그래픽카드, 모니터 주사율 정보 가져오기
		...
		{
			if (displayModeList[i].Width == (unsigned int)winWidth && displayModeList[i].Height == (unsigned int)
				winHeight)
			{
...
#pragma region swapChain Description
		...
		mSwapChainDesc.BufferDesc.RefreshRate.Numerator   = bVsync ? numerator : 0;
		mSwapChainDesc.BufferDesc.RefreshRate.Denominator = bVsync ? denominator : 1;
...
#pragma region Direct3D Device, DeviceContext 초기화
		...
		if (FAILED(D3D11CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, 0, &mFeatureLevel, 1,
			D3D11_SDK_VERSION, &mSwapChainDesc, &mSwapChain, &mD3D, NULL, &mD3DDC)))
		{
			return false;
		}
...
#pragma region Depth Stencil Buffer Description
		...
		mD3DDC->OMSetDepthStencilState(mDepthStencilState, 1);
...
#pragma endregion Depth Stencil State Description
		...
		mD3DDC->OMSetRenderTargets(1, &mRenderTargetView, mDepthStencilView);
...
#pragma region Resterizer Description
		...
		mD3DDC->RSSetState(mRasterState);
...
#pragma region Viewport Setting for Rendering
		...
		mD3DDC->RSSetViewports(1, &mViewport);
...
#pragma endregion Viewport Setting for Rendering
	...
	void Direct3D::BeginScene(float R, float G, float B, float A)
	{
		...
		mD3DDC->ClearRenderTargetView(mRenderTargetView, color);

		mD3DDC->ClearDepthStencilView(mDepthStencilView, D3D11_CLEAR_DEPTH, 1.0f, 0);
	}

	void Direct3D::EndScene()
	{
		mSwapChain->Present(bVsync ? 1 : 0, 0);
	}
	...
	HRESULT Direct3D::ResizeSwapChain(UINT Width, UINT Height)
	{
		...
		mD3DDC->OMSetRenderTargets(0, nullptr, nullptr);
